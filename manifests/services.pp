...
class paperless_ngx::services inherits paperless_ngx {
  ...
  each($systemd_services) |$service| {
    ...
    file_line{ "set user for ${service}":
      path => "/etc/systemd/system/${service}",
      match => '^User=',
      line => "User=${paperless_system_user}",
    }   
    file_line{ "set group for ${service}":
      path => "/etc/systemd/system/${service}",
      match => '^Group=',
      line => "Group=${paperless_system_group}",
    } 
  ...
  file_line{ "set ExecStart for paperless-consumer.service":
    ...
    match => "^ExecStart=",
    line => "ExecStart=${paperless_virtualenv}/bin/python3 manage.py document_consumer",
  ...
  file_line{ "set ExecStart for paperless-scheduler.service":
    ...
    match => "^ExecStart=",
    line => "ExecStart=${paperless_virtualenv}/bin/python3 manage.py qcluster",
  ...
  file_line{ "set ExecStart for paperless-webserver.service":
    ...
    match => "^ExecStart=",
    line => "ExecStart=${paperless_virtualenv}/bin/gunicorn -c ${paperless_directory}/gunicorn.conf.py paperless.asgi:application",
